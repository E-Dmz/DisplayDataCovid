...
def figure_line(row, nrow, axs, d, column_to_plot, regions, graph_options):
    ...
    plot_three_curves(ax, d, "France", column_to_plot, hline = hline, **graph_options[column_to_plot])
    format_graph(ax,
    ...
    for i, region in enumerate(regions):
        col = 2 + i
        ax = axs[row * ncol + col]
        plot_three_curves(ax, d, region, column_to_plot, hline = hline, **graph_options[column_to_plot])
        format_graph(ax,
                x_axis = ('regular' if (row == nrow -1 ) else 'without'), 
                y_labels = ('to_the_left' if (col == 0) else 'to_the_right' if (col == ncol - 1) else 'without'),
                dense = True,
...
def fig_type1(d, regions, fig_id, graph_options = graph_options):

    ncol = len(regions) + 2
    nrow = 7
    fig, axs = plt.subplots(nrow, ncol, figsize = (3 * ncol, 1.5 * nrow))
    axs = axs.ravel() 
...
{fig_id} - {decomposition_clage}'
        .format(fig_id = fig_id, date = TODAY[1], decomposition_clage = decomposition_clage),
                x = 0.035, y = 0, 
    ...
    save_output(fig, dir_PNG, fig_id)
    save_output(fig, '../Output/Type1/', fig_id)
